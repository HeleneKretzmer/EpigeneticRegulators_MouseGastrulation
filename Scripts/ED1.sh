...
data_left <- read.table('ED1b_PCA_Clustering.tsv', header=T, row.names=1)
data_center <- read.table('ED1b_PCA_Unstable.tsv', header=T, row.names=1)
data_right <- read.table('ED1b_PCA_Filtering.tsv', header=T, row.names=1)
...
data <- read.table('ED1c_WT_sex_expr.tsv', header=T)
...
# ED 1e left
```{r}
data <- read.table('ED1d_Marker_count_vs_unique.tsv', header=T)
...
data <- read.table('ED1d_Marker_count_vs_top30.tsv', header=T)
...
data <- read.table('ED1f_WTClosestCluster.tsv', header=T)
...
# ED 1h left
```{r}
data <- read.table('ED1h_CellState_prevalence.tsv', header=T)
...
WT = sc.read('ED1i_WT_velocity.h5ad', sparse=True)
