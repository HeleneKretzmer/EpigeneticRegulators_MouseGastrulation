...
data <- read.table('KO_embryo_staging.tsv', header=T)
...
data <- read.table('CellState_FC.tsv', header=T, row.names=1)
...
CGI_Epi <- read.table('CGI_Epi.tsv', header=T)
CGI_Exe <- read.table('CGI_ExE.tsv', header=T)
